...
-- ORDER BY는 결과물의 개수는 종류에는 영향을 미치지 않지만
...
-- 아래 코드는 생년으로 오름차순을 하되, 동점이면 키 내림차순으로 나열합니다.
SELECT * FROM userTbl ORDER BY birth_year ASC, height DESC;

-- 만약에 문자라면 아스키코드 순으로 ORDER BY 정렬이 들어갑니다.
-- user_name으로 ORDER BY를 걸어보세요.
SELECT * FROM userTbl ORDER BY user_name DESC;

-- 지역 가나다라 역순으로 나열해주시되, 만약 지역이 같다면 이름 오름차순으로 나열해주세요.
...
SELECT DISTINCT birth_year FROM userTbl;
...
-- 1. 날짜로 정렬 ,  2. 정렬된 17만개 중 10개를 뽑기
SELECT * FROM employees ORDER BY hire_date DESC limit 10;

-- 일정 데이터셋을 먼저 얻어놓고 그 내부적으로 정렬시킬때는 서브쿼리를 씁니다.
-- 1. 17만개 중 10개를 먼저 뽑기  ,  2. 그 10개를 정렬하기
...
-- 컬럼명을 바꿀 땐 as '내용' 을 사용하면 됩니다. 그냥 내용만 적어도 무방하나
-- 가독성을 위해 as를 쓰길 권장합니다.
SELECT user_id, sum(price * amount) as 'total' FROM buyTbl GROUP BY user_id;

-- 각 유저별 구매 물품의 평균가를 구해보세요. 평균은 avg()로 처리합니다.
...
SELECT user_id, sum(price * amount) as '총 구매액' 
...
-- 데이터 제어언어로 특정 계정에 대한 권한을 부여하거나 뺐을 때 사용합니다.
-- GRANT, REVOCE, DENY 등이 있습니다.



-- 테이블 생성하기
-- 테이블명 : testtbl1, id int, user_name varchar(3), age int 형식
...
-- 테이블명 뒤에 컬럼 정보를 적지 않으면, CREATE TABLE시 설정한
...
INSERT INTO testtbl1(user_name, age, id) VALUES ('김자바', 10, 4);
