...
-- 제 1정규화 (회원별 구매내역이라는 테이블을 만들시 회원별을 구분할 수 있는 회원정보
-- 테이블을 따로 저장해놓고 필요할 때 쓰는 개념)
-- *외래 키 설정 : 앞으로 회원별 구매내역의 회원번호는, 회원 정보 테이블의 회원번호에 
-- 존재하는 값만 가져오겠다 라는 의미

-- 테이블을 생성합니다
-- 테이블 생성 시 적힌 제약조건과 컬럼명을 활용해 만들어주세요.
...
CREATE TABLE userTbl(
    ...
    height INT(3),
    reg_date DATE -- TIMESTAMP는 DATETIME은 날짜 및 시간까지 나타냄
    );
    
-- AUTO_INCERMENT(자동 증가)
-- INT(정수)자료형이 붙은 컬럼에만 붙일 수 있고, 이게 붙은 컬럼은 
-- 자동으로 1씩 증가하는 숫자를 배정받습니다. 시작은 1입니다.
...
-- 회원부터 집어넣었음.
INSERT INTO userTBl VALUES('KJV', '김자바', 2000, '경기', '01012345678', 180, '2020-3-5');
INSERT INTO userTBl VALUES('SHM', '손흥만', 1988, '홍천', '01023459767', 182, '2020-7-7');
INSERT INTO userTBl VALUES('EM', '일리마스크', 1999, '서울', '01048942345', 175, '2020-12-5');
SELECT * FROM userTBl;

-- 가입한 회원이 물건을 산 이력이 남도록 하겠습니다.
INSERT INTO buyTBl VALUES(null, 'KJV', '기계식키보드', '주변기기', 200000, 1);
-- order_number는 값을 null로 주면 AUTO_INCREMENT에 의해 자동으로 1씩 증가된 숫자를 배정받음
INSERT INTO buyTBl VALUES(null, 'KJV', '듀얼모니터', '전자기기', 20000, 1);
-- 없는 회원을 아이디로 INSERT를 시도해보겠습니다.
